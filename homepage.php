...
$(function() {
	var d=new Date();
	  ...
	  if(diff>0)
	  {
		  warningAlert("Please do not enter future date");
	  ...
	  if(diff1 <0) //check if the expected date is a future date and not past date
	  {
		  ...
		  warningAlert("Please enter a date after today");
		  $("#dateborrowedlent").val('');
		  $("#expectedreturndate").val('');
		  dateAsObject=null;
		  dateAsObject1=null;
	  ...
	  {
		  warningAlert("Please enter the lent date");
		  $("#dateborrowedlent").val('');
		  $("#expectedreturndate").val('');
		   dateAsObject=null;
		  dateAsObject1=null;
...
<div id="displayshareAndRemind" align="right" style="color:red"></div>  
...
if((strcmp($row["email"], $email1))!=0)
	...
	//to-do part queries
	$sql1="SELECT signup.firstname,shareandremind.itemborrowedlent,shareandremind.owner,shareandremind.expectedreturndate
	     FROM signup 
	     INNER JOIN shareandremind 
	     ON shareandremind.remindto = signup.email WHERE
	 		signup.firstname = 'Manasa Ranjit'";
			//echo $sql1;
	...
	//to-do part queries
	$sql4="SELECT signup.firstname,shareandremind.itemborrowedlent,shareandremind.owner,shareandremind.expectedreturndate
	     FROM signup 
	     INNER JOIN shareandremind 
	     ON shareandremind.remindto = signup.email WHERE
